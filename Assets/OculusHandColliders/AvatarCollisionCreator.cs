...
{ 
    ...
    [Tooltip("Choose which layer colliders belong to")]
    ...
    void FindAndModifyBoneTransforms(Hand hand)
    {
        ...
        //custom hands
        //string handString = hand.isRightHand ? "hands:b_r_hand" : "hands:b_l_hand";
    ...
    void CreateCollider(GameObject bone)
    {
            ...
            bone.transform.name.Contains("pinky"))
        {
            if (!bone.transform.name.EndsWith("0"))
            {
                ...
                int layer = -1;
                layer = LayerMask.NameToLayer(colliderLayer);
                if(layer > 0) bone.transform.gameObject.layer = LayerMask.NameToLayer(colliderLayer);
